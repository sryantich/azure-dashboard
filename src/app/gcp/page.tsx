...
import GcpCostChart from "../../components/GcpCostChart";
...
import { useGcpStatus, GCP_REGION_CONNECTIONS } from "../../hooks/useGcpStatus";
import { useGcpActivityFeed } from "../../hooks/useGcpActivityFeed";
import { useGcpIncidents } from "../../hooks/useGcpIncidents";
import { useGcpResourceHealth } from "../../hooks/useGcpResourceHealth";
...
function GcpDashboard() {
  ...
  const metrics = useGcpMetrics(40, 2000);
  const gcp = useGcpStatus(30000);
  const events = useGcpActivityFeed(50);
  const incidents = useGcpIncidents(60000);
  const resourceHealth = useGcpResourceHealth(10000);
    ...
    >
      ...
      >
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-white mb-1 flex items-center gap-2.5">
            <Cpu className="w-7 h-7 sm:w-8 sm:h-8" style={{ color: "#34d399" }} />
            GCP Live Ops
          </h1>
          <p className="text-slate-500 text-xs sm:text-sm font-medium tracking-wide">
            Real-time Google Cloud infrastructure & performance monitoring
          </p>
        </div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <MetricCard
          ...
          format={fmtInt}
          changeLabel={c.runningResources > 1380 ? `+${c.activeResources - 1400}` : `${c.activeResources - 1400}`}
          trend={c.activeResources >= 1400 ? "up" : "down"}
          trendIsGood={true}
          icon={Server}
        ...
        <MetricCard
          ...
          format={fmtReqS}
          unit="req/s"
          changeLabel={c.throughput > 3400 ? `+${((c.throughput / 3400 - 1) * 100).toFixed(1)}%` : `${((c.throughput / 3400 - 1) * 100).toFixed(1)}%`}
          trend={c.throughput > 3400 ? "up" : c.throughput < 2600 ? "down" : "neutral"}
          trendIsGood={true}
          icon={Gauge}
        ...
        <MetricCard
          ...
          format={fmtMs}
          unit="ms"
          changeLabel={c.avgLatency > 30 ? `+${(c.avgLatency - 24).toFixed(0)}ms` : `${(c.avgLatency - 24).toFixed(0)}ms`}
          trend={c.avgLatency > 35 ? "up" : c.avgLatency < 18 ? "down" : "neutral"}
          trendIsGood={false}
          icon={Timer}
      ...
      {/* ── Status + Feed Row ── */}
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <ServiceStatusGrid services={gcp.services as never} title="GCP Service Health" />
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 gap-4">
        <NetworkMap
          regions={gcp.regions}
          connections={GCP_REGION_CONNECTIONS}
          title="Global GCP Regions"
          subtitle={`${gcp.regions.length} regions monitored \u00b7 ${GCP_REGION_CONNECTIONS.length} network links`}
        />
      </motion.div>
    </motion.div>
  );
